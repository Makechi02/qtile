...
class Keybindings:
    ...
    def init_keys_groups(self, group_names):
        ...
        for i in range(len(group_names)):
            index = str(i + 1)
            group_keys += [
                Key([MOVEMENT_KEY], index, lazy.group[group_names[i]].toscreen()),
                Key([MOVEMENT_KEY, SWAP_GROUP_KEY], index, lazy.window.togroup(group_names[i], switch_group = True))
            ]
